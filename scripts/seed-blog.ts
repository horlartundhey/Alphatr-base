/**
 * One-time seed script — inserts all 12 blog posts from the static backup into MongoDB.
 * Run from the server/ folder:
 *   npx ts-node --esm scripts/seed-blog.ts
 *   OR (if using tsx):
 *   npx tsx scripts/seed-blog.ts
 *
 * Posts that already exist (by slug) are SKIPPED — safe to re-run.
 */
...
// ── Inline schema (avoids import issues with the lib/ path) ──────────────────
...
<p>GA4 uses "events" where Universal Analytics used "goals." Prioritise events that signal clear business intent: form_submit, purchase, schedule_call, file_download, and phone_call_click.</p>
...
<p>Use GA4's DebugView with the GA Debugger Chrome extension enabled. Walk through every conversion path on your site and confirm events are firing correctly, with the right parameters, at the right moment.</p>`,
  },
  {
    ...
    content: `<p>The Meta Pixel was once the backbone of Facebook and Instagram advertising. Then Apple introduced App Tracking Transparency in iOS 14.5, and everything changed. Cookie-based tracking became unreliable. Conversions disappeared from the platform. ROAS calculations became guesswork.</p>
<h2>Why the Pixel Alone Is No Longer Enough</h2>
<p>The standard Meta Pixel fires from the user's browser. When a user has iOS privacy restrictions, an ad blocker, or a privacy-focused browser, the Pixel signal is either blocked or degraded. Studies suggest that in privacy-regulated markets, client-side Pixel tracking underreports conversions by 30–60%.</p>
...
<p>If you're on Shopify, WooCommerce, or another supported platform, Meta has native CAPI integrations that handle deduplication automatically.</p>
<h3>Option B: Gateway via GTM Server-Side (Recommended)</h3>
<p>For custom setups, implement CAPI through a server-side GTM container. This gives you full control and works across any platform.</p>
<h2>Deduplication: The Critical Step Most Businesses Skip</h2>
<p>Always pass matching event_id values from both the browser Pixel and CAPI for the same event. Meta uses this to deduplicate correctly and avoid double-counting conversions.</p>`,
  },
  {
...
<p>Job titles on LinkedIn are inconsistent. Unless you're targeting a very specific title, use Job Function + Seniority instead. This gives you broader reach while maintaining relevance.</p>
<h3>Company Size Segmentation</h3>
<p>Create separate campaigns for SMBs, mid-market, and enterprise with tailored messaging. An ad that resonates with a 10-person startup will not speak to a procurement lead at a 500-person company.</p>
...
<p>Boost organic posts as Thought Leadership Ads. These blend seamlessly into the feed and build brand credibility with your target audience over time.</p>
<h3>Document Ads</h3>
...
<p>Schedule automated email delivery to stakeholders every Monday morning. In Looker Studio, go to Share → Schedule Email Delivery to configure this. This single habit replaces hours of weekly reporting.</p>`,
  },
  {
